...
export const CoinInfoReducer = (
	state = initialCoinInfo,
	action: CoinInfoAction
) => {
	switch (action.type) {
		...
		case CoinInfoActionTypes.SET_NAME_COIN:
			return {
				...state,
				name: action.payload,
				symbol: getSymbol(action.payload)
			}
		case CoinInfoActionTypes.SET_RATE_COIN:
			return {
				...
				MarketInfo: {
					...
					change: {
						price_change_24h:
							action.payload - state.MarketInfo.openPrice,
						price_change_percentage_24h:
							((action.payload - state.MarketInfo.openPrice) /
								state.MarketInfo.openPrice) *
							100
					}
				}
			}
