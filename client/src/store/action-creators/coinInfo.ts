...
export const fetchCoinInfo = (name = '') => {
    ...
    return async (dispatch: Dispatch<CoinInfoAction>) => {
         try{
            ...
            const responseRate = await axios.get(`https://api.binance.com/api/v3/ticker`, {
                params: {symbol: getSymbol(name).toUpperCase() + "USDT"}
            })
            const rate = {
                rate: responseRate.data.lastPrice,
                change: {price_change_24h: responseRate.data.priceChange, price_change_percentage_24h: responseRate.data.priceChangePercent},
                openPrice: responseRate.data.openPrice
            }
            
            dispatch({type: CoinInfoActionTypes.FETCH_COININFO_SUCCESS, payload: {...info, ...rate}})
         }catch(e){
            dispatch({type: CoinInfoActionTypes.FETCH_COININFO_ERROR, 
                      payload: 'Error'
            })
         }
    }
