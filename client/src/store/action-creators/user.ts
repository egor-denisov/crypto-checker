    ...
    return async (dispatch: Dispatch<UserAction>) => {
        try{
           ...
           if(!isExist(data.id)){
            ...
            ? dispatch({type: UserActionTypes.FETCH_CHECKUSER_ERROR, payload: 'Wrong password'})
            : dispatch({type: UserActionTypes.FETCH_CHECKUSER_ERROR, payload: 'User not found'})
           }else{
            const washlistAndWallet = await axios.post(`http://localhost:1234/api/washlist_and_wallet`, {
           ...
           dispatch({type: UserActionTypes.FETCH_CHECKUSER_ERROR, 
                     payload: 'Error'
           })
        }
   }
...
export const ChangeWashlist = (coin: string, washlist: WashListType, washlist_id: number, type: string) => {
   ...
   return async (dispatch: Dispatch<UserAction>) => {
      try{
         ...
         const response = await axios.post(`http://localhost:1234/api/change_washlist`, {
            washlist_id: washlist_id,
            washlist: newWashlist
         })
         if(response.data.update) dispatch({type: UserActionTypes.UPDATE_WASHLIST, payload: newWashlist})
      }catch(e){}
   }
...
export const clearWashlist = (washlist_id: number) => {
   return async (dispatch: Dispatch<UserAction>) => {
      try{
         const response = await axios.post(`http://localhost:1234/api/change_wallet`, {
            washlist_id: washlist_id,
            washlist: []
         })
         if(response.data.update) dispatch({type: UserActionTypes.UPDATE_WASHLIST, payload: []})
      }catch(e){}
   }
}

export const ChangeWallet = (coin: string, data: WalletElementType, wallet: WalletType, wallet_id: number, type: string) => {
   ...
   return async (dispatch: Dispatch<UserAction>) => {
      try{
         ...
         if(response.data.update) dispatch({type: UserActionTypes.UPDATE_WALLET, payload: newWallet})
      }catch(e){
         console.log(e)
      }
   }
