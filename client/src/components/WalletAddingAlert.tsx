...
const WalletAddingAlert: FC<props> = ({coin, setVisible, IsWithInput}) => {
    ...
    const changeCountOrPrice = (e: React.FormEvent<HTMLInputElement>, field: keyof typeof coinWalletData ) => {
        const value = e.currentTarget.value
        if(value === "") setCoinWalletData({...coinWalletData, [field]: 0})
        else setCoinWalletData({ ...coinWalletData, [field]: Number(e.currentTarget.value)})
    }
    const setValue = (value: string) => {
        setAddingError("")
        setCoinWalletData({...coinWalletData, coin: value, 
        ...
        <MyModal setVisible={setVisible} className="modal-wallet">
            ...
            <div className="form">
                <div className="name">
                    <p>Coin:</p>
                    {IsWithInput
                    ? <InputWithMatches value={coinWalletData.coin} 
                                        setValue={setValue}
                                        condition={addingError === walletErrors.nameError ? "error" : ""}
                                        />
                    : <MyInput value={coin} 
                               disabled
                               condition={addingError === walletErrors.nameError ? "error" : ""}
                    ...
                    <div className='error'>{addingError === walletErrors.nameError ? addingError : ""}</div>
                </div>
                <div className="count">
                    <p>Count:</p>
                    <MyInput value={coinWalletData.count} 
                             onChange={(e: React.FormEvent<HTMLInputElement>) => changeCountOrPrice(e, "count")}
                             onKeyDown={onEnter}
                             condition={addingError === walletErrors.countError ? "error" : ""}
                    />
                    <div className='error'>{addingError === walletErrors.countError ? addingError : ""}</div>
                </div>
                <div className="price">
                    <p>Price:</p>
                    <MyInput value={coinWalletData.price} 
                             onChange={(e: React.FormEvent<HTMLInputElement>) => changeCountOrPrice(e, "price")}
                             onKeyDown={onEnter}
                             condition={addingError === walletErrors.priceError ? "error" : ""}
                    />
                    <div className='error'>{addingError === walletErrors.priceError ? addingError : ""}</div>
                </div>
            </div>
