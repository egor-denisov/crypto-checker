...
const LeftWalletMenu: FC<props> = ({
	...
	const defaultContexMenu = {
		coin: '',
		vizible: vizibleContexMenu,
		X: 0,
		Y: 0
	}
	...
	useEffect(
		() => setContextMenu({ ...ContextMenu, vizible: vizibleContexMenu }),
		[vizibleContexMenu]
	)
	return (
		<>
			{ContextMenu.vizible && (
				<CoinContexMenu
					goEdit={() => editCoinInWallet(ContextMenu.coin)}
			...
			<div className="left-wallet-menu">
				...
				<div className="icons-and-add">
					...
					<div className="buttons">
						{!editMode && isExist(data.wallet_keys[0]) && (
				...
				<div className="coins">
					{isExist(data.wallet_keys[0]) ? (
						data.wallet_keys.map((coin) => {
							const currentPrice =
								data.wallet[coin].count * rates[coin].rate
							return (
								<div
									className={uniteClasses([
										'item',
										activePart === coin && 'active'
									])}
									onClick={() => setActivePart(coin)}
									onContextMenu={(e: React.MouseEvent) =>
										callCoinContextMenu(e, coin)
									}
									key={coin}
									style={{
										borderRight:
											'5px solid' + getColor(coin) + 'b3'
									}}
								>
