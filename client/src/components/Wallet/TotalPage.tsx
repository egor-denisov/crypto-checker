			...
			callbacks: {
				label: (context: any) => {
					...
					if (context.parsed !== null) {
						label += `${
							context.label[0]?.toUpperCase() +
							context.label.slice(1)
						}: ${context.parsed}%`
					}
					return label
				},
...
const TotalPage: FC<props> = ({ walletPercentage, currentBalance }) => {
	...
	useEffect(() => {
		if (!isExist(data.wallet_keys[0]))
			...
			const labels = Object.keys(data.wallet).sort((a, b) =>
				rates[a].rate * data.wallet[a].count >
				rates[b].rate * data.wallet[b].count
					? -1
					: 1
			)
			labels.forEach((el) => {
			...
			const datasets = [
				{
					...defaultChartData.datasets[0],
					data: walletPercentage.sort((a, b) => (a < b ? 1 : -1)),
					backgroundColor: Object.values(coinAndColor)
				}
			]
			setChartData({
				...defaultChartData,
