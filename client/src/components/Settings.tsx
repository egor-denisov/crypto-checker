...
const Settings: FC<props> = ({visible, setVisible}) => {
    const [setting, setSetting] = useState({language: 'EN', currency: 'USD', colorMode: 'Light'})
    const changeLanguage = () => setSetting({...setting, language: setting.language === 'EN' ? 'RU' : 'EN'})
    const changeCurrency = () => setSetting({...setting, currency: setting.currency === 'USD' ? 'RUB' : 'USD'})
    const changeColorMode = () => setSetting({...setting, colorMode: setting.colorMode === 'Dark' ? 'Light' : 'Dark'})
    if(!visible) return <></>
    return (
        <MyModal setVisible={setVisible} className="setting">
            <div className="title"><SettingsSVG/>Settings</div>
            <div className='row language'>
                <p className='caption'>Language: </p>
                <div className='value'>
                    <MyToogle firstItem='EN' 
                              secondItem='RU' 
