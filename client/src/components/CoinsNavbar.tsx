	...
	return (
		<div className="coins-navbar">
			<div className="selectShowing">
				<GridSVG
					onClick={() => setInputType('cards')}
					className={inputType === 'cards' ? 'active' : ''}
				/>
				<RowsSVG
					onClick={() => setInputType('table')}
					className={inputType === 'table' ? 'active' : ''}
				/>
			</div>
			...
			<div className="selectSorting">
				<MySelect
					value={typeOfSorting}
					onChange={(e: any) => setTypeOfSorting(e.target.value)}
				>
					<option value="Type of sorting" disabled>
						Type of sorting
					</option>
					{sortings.map((sorting) => {
						return (
							<option value={sorting} key={sorting}>
								{sorting}
							</option>
						)
					})}
				...
				<div className="toggleDirection">
					<ArrowDownSVG
						onClick={() => setSortingDirection('descending')}
						className={
							sortingDirection === 'descending' ? 'active' : ''
						}
					/>
					<ArrowUpSVG
						onClick={() => setSortingDirection('ascending')}
						className={
							sortingDirection === 'ascending' ? 'active' : ''
						}
					/>
				</div>
			</div>
